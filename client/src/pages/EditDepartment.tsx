...
import { Building2, MapPin, Phone, CheckCircle, ArrowLeft, ArrowRight, Users, Settings, Loader2 } from 'lucide-react';
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        form.reset({
          ...
          type: department.type || 'investigation',
          parentDepartment: department.parentDepartment?._id || department.parentDepartment || '',
          location: {
            address: {
              street: department.location?.address?.street || '',
              city: department.location?.address?.city || '',
              state: department.location?.address?.state || '',
              country: department.location?.address?.country || 'Somalia',
              postalCode: department.location?.address?.postalCode || ''
            },
            phone: department.location?.phone || '',
            email: department.location?.email || ''
          },
          contactInfo: {
            phone: department.contactInfo?.phone || '',
            email: department.contactInfo?.email || '',
            fax: department.contactInfo?.fax || '',
            website: department.contactInfo?.website || '',
            emergencyContact: department.contactInfo?.emergencyContact || ''
          },
          status: {
            isActive: department.status?.isActive ?? true,
            establishedDate: department.status?.establishedDate ? formatDateForInput(department.status.establishedDate) : ''
          },
            ...
            <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-xl">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex justify-between pt-4 border-t border-gray-200">
                  ...
                  >
                    <span className="text-white">{currentStep === steps.length - 1 ? 'Update Department' : 'Next'}</span>
