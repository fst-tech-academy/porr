...
router.get('/categories/list', protect, authorize(['admin', 'manager', 'officer']), async (req, res) => {
  try {
    const categories = [
...
router.get('/severities/list', protect, authorize(['admin', 'manager', 'officer']), async (req, res) => {
  try {
    const severities = [
